...
mp.attachmentMngr = {
  ...
  addFor: function (entity, id) {
    if (this.attachments.hasOwnProperty(id)) {
      if (!entity.__attachmentObjects.hasOwnProperty(id)) {
        ...
        this.watitngAttachment[id] = { entity: entity, object: object, attInfo: attInfo };
  ...
  attachFor: function (id) {
    try {
      let entity = this.watitngAttachment[id].entity;

      let object = this.watitngAttachment[id].object;

      let attInfo = this.watitngAttachment[id].attInfo;
      ...
      this.watitngAttachment[id] = undefined;
    } catch (e) {
      mp.game.graphics.notify(`Static Attachments Error: 0x${id.toString(16)}`);
    }
  },
  ...
  initFor: function (entity) {
    for (let attachment of entity.__attachments) {
      this.attachmentMngr.addFor(entity, attachment);
    }
  },

  shutdownFor: function (entity) {
    for (let attachment in entity.__attachmentObjects) {
      this.attachmentMngr.removeFor(entity, attachment);
    }
  },
...
mp.events.add("entityStreamIn", (entity) => {
  if (entity.attachId) {
    mp.attachmentMngr.attachFor(entity.attachId);
  }
